...
public enum CoreMLComputePlanAnalyzer {
  ...
  private static func analyzeAvailable(
    ...
    AppLogger.app.info(
      "Core ML compute-plan analysis started: \(compiledURL.lastPathComponent, privacy: .public) [\(mode.rawValue, privacy: .public)]"
    )

    // MLComputePlan already owns the model structure that is required for
    // operator/device inspection. Build 30 originally loaded the complete
    // MLModel a second time only to read stateDescriptionsByName. For large
    // stateful transformer graphs that can cause another expensive execution-
    // plan build and make the UI look permanently busy. The analyzer therefore
    // performs exactly one heavy Core ML load now.
    ...
    AppLogger.app.info(
      "Core ML compute-plan analysis completed: \(accumulator.totalOperations) operations in \(milliseconds(from: elapsed), privacy: .public) ms"
    )
  ...
  private struct Accumulator {
    ...
    mutating func record(
      ...
      // Stateful ML Program graphs generated by Core ML Tools use explicit
      // state operations. This avoids loading the whole MLModel again merely
      // for UI diagnostics. The authoritative runtime state handling remains
      // MLModel.modelDescription.stateDescriptionsByName in the prediction
      // runner, where it is actually needed.
      let normalizedName = name.lowercased()
      if normalizedName == "read_state"
        || normalizedName == "coreml_update_state"
        || normalizedName == "update_state"
      {
        stateful = true
      }
